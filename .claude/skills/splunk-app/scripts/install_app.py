...
def main():
    ...
    parser.add_argument('source', help='App package path (.tar.gz/.tgz/.spl) or Splunkbase app ID')
    parser.add_argument('--name', '-n', help='App name (auto-detected from package if not specified)')
    parser.add_argument('--update', '-u', action='store_true', help='Update if app already exists')
    ...
    if not is_file and not args.source.isdigit():
        print_warning(f"Source '{args.source}' is not a valid file path or Splunkbase app ID")
        return
    ...
    if is_file:
        ...
        data = {
            'name': args.name or source_path.stem.replace('.tar', '').replace('.spl', ''),
        ...
        response = client.post(
            '/services/apps/local',
            data=data,
            operation='install app from Splunkbase'
        )
