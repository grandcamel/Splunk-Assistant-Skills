...
def main():
    ...
    if args.output == 'json':
        ...
        details = [
            ...
            {'Property': 'Description', 'Value': (content.get('description', 'N/A') or 'N/A')[:60]},
            {'Property': 'Visible', 'Value': 'Yes' if content.get('visible') else 'No'},
            {'Property': 'Disabled', 'Value': 'Yes' if content.get('disabled') else 'No'},
            {'Property': 'Configured', 'Value': 'Yes' if content.get('configured') else 'No'},
            {'Property': 'Check for Updates', 'Value': 'Yes' if content.get('check_for_updates') else 'No'},
        ]
