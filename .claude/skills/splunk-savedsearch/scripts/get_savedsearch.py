...
def main():
    ...
    if args.output == 'json':
        ...
        details = [
            ...
            {'Property': 'Cron Schedule', 'Value': content.get('cron_schedule', 'Not scheduled')},
            {'Property': 'Is Scheduled', 'Value': 'Yes' if content.get('is_scheduled') else 'No'},
            {'Property': 'Disabled', 'Value': 'Yes' if content.get('disabled') else 'No'},
            {'Property': 'Next Run', 'Value': content.get('next_scheduled_time', 'N/A')},
            {'Property': 'Actions', 'Value': content.get('actions', 'None')},
            {'Property': 'Alert Type', 'Value': content.get('alert_type', 'N/A')},
            {'Property': 'Dispatch Earliest', 'Value': content.get('dispatch.earliest_time', 'N/A')},
            {'Property': 'Dispatch Latest', 'Value': content.get('dispatch.latest_time', 'N/A')},
        ]
