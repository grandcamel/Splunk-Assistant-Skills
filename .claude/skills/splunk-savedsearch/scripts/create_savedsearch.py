...
def main():
    ...
    parser.add_argument('--cron', help='Cron schedule (e.g., "0 6 * * *" for 6 AM daily)')
    parser.add_argument('--is-scheduled', action='store_true', help='Enable scheduling')
    parser.add_argument('--alert-type', choices=['always', 'number of events', 'number of hosts', 'number of sources'],
                        help='Alert trigger condition')
    parser.add_argument('--alert-threshold', help='Alert threshold (e.g., "> 10")')
    parser.add_argument('--actions', help='Comma-separated actions: email,webhook,script')
