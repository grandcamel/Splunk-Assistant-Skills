...
def main():
    ...
    response = client.get(
        f'/services/alerts/fired_alerts/{args.name}',
    ...
    if args.output == 'json':
        ...
        details = [
            ...
            {'Property': 'Saved Search', 'Value': content.get('savedsearch_name', 'N/A')},
            {'Property': 'Severity', 'Value': content.get('severity', 'N/A')},
            {'Property': 'Trigger Time', 'Value': content.get('trigger_time', 'N/A')},
            {'Property': 'Triggered Count', 'Value': content.get('triggered_alert_count', 0)},
            {'Property': 'Expiration Time', 'Value': content.get('expiration_time', 'N/A')},
            {'Property': 'Digest Mode', 'Value': 'Yes' if content.get('digest_mode') else 'No'},
        ]
