...
class TestQueryCollection:
    ...
    def test_query_collection_no_filter(
        self,
        mock_format,
        mock_print,
        mock_get_client,
    ...
    def test_query_collection_json_output(
        self,
        mock_format_json,
        mock_get_client,
        mock_splunk_client,
        ...
        with patch(
            "sys.argv", ["query_collection.py", "test_collection", "--output", "json"]
        ):
            main()
    ...
    @patch("query_collection.get_splunk_client")
    @patch("query_collection.print_success")
    def test_query_collection_with_limit(
        self, mock_print, mock_get_client, mock_splunk_client
    ):
        ...
        with patch(
            "sys.argv", ["query_collection.py", "test_collection", "--limit", "10"]
        ):
            main()
    ...
    @patch("query_collection.get_splunk_client")
    @patch("query_collection.print_success")
    def test_query_collection_with_fields(
        self, mock_print, mock_get_client, mock_splunk_client
    ):
        ...
        with patch(
            "sys.argv",
            ["query_collection.py", "test_collection", "--fields", "username,email"],
        ):
            main()
