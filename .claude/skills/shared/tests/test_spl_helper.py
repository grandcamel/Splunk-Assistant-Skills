...
class TestValidateSplSyntax:
    ...
    def test_valid_complex(self):
        is_valid, issues = validate_spl_syntax('index=main | stats count by host | sort -count')
        assert is_valid is True
...
class TestEstimateSearchComplexity:
    ...
    def test_medium(self):
        assert estimate_search_complexity('index=main | stats count by host | sort -count') == 'medium'
    ...
    def test_complex_join(self):
        assert estimate_search_complexity('index=main | join type=left host [search index=other]') == 'complex'
